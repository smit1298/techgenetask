...
const OrderPickUpList = () => {
      ...
      category.sub_categories.map((subcategory) => {
        if (subcategory.Merchandize_category_id == categoryID) {
          subcategory.sub_categories.map((sub) =>
            subCat.push({
              sub_cat_name: sub.Merchandize_category_name,
              sub_cat_id: sub.Merchandize_category_id
            })
          );
        }
      });
  ...
  useEffect(() => {
    if (!getData(categoryStore)?.data) {
      doGet(
        setCategories,
        "api/multi_categories?token=" + process.env.APPLICATION_TOKEN
      );
    ...
    <div>
      <div className="pt-4 space-y-4">
          ...
          {categories?.length > 0
            ? categories?.map((category) =>
                category.sub_categories.map((subCat) => (
                  <button
                    key={
                      subCat.Merchandize_category_id +
                      category.Merchandize_category_id
                    }
                    className="font-bold py-2 flex border-sub border-solid border rounded-xl px-4 items-center text-lg leading-product tracking-loyal text-center"
                    onClick={() => {
                      getNestedCategories(subCat?.Merchandize_category_id);
                    }}
                  >
                    <p className="">
                      {subCat?.Merchandize_category_name.toUpperCase()}
                    </p>
                  </button>
                ))
              )
            ...
            <button
              ...
              onClick={() => {
                fetchCategoryItems(
                  ...
                  setArrivalItems,
                  "api/pickup_items_categories/" + subcategory?.sub_cat_id
                );
              }}
            >
              {subcategory?.sub_cat_name.toUpperCase()}
        ...
        <div className="flex flex-col">
          ...
          <div className="flex mx-auto flex-wrap pl-[5%] gap-3 sm:px-3 sm:gap-4 sm:w-[85%]">
            {arrivalItems?.data ? (
              arrivalItems?.data?.map((item, index) =>
                index <= 12 ? (
                  <OrderPickUpItems key={index} card={item} />
                ) : (
